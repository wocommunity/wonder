...
import com.frontbase.jdbc.FBJConnection;
import java.io.*;
...
public class FrontbasePlugIn extends JDBCPlugIn {
    ...
    public void updateLOBs(JDBCChannel channel, JDBCExpression expression, NSDictionary dictionary, EOEntity entity) {
        ...
        try {
            FBJConnection con = (FBJConnection) ((JDBCContext) channel.adaptorContext()).connection();
    ...
    String getLobHandle(FBJConnection con, Object attribute, Object value) throws SQLException{
        switch (internalTypeForExternal(((EOAttribute) attribute).externalType())) {
            case FB_BLOB:
                return con.writeBLOB(((NSData) value).bytes());
            case FB_CLOB: 
                return con.writeCLOB((String) value);
            default:
                return "NULL";
        }
    }
    ...
    public static class FrontbaseSynchronizationFactory extends EOSynchronizationFactory {
        ...
        public String columnTypeStringForAttribute(EOAttribute eoattribute) {
            ...
            if (nsdictionary1 == null)
                throw new JDBCAdaptorException("Unable to find type information for external type '" + s + "' in attribute '" + eoattribute.name() + 
                                               "' of entity '" + ((EOEntity) eoattribute.parent()).name() + "'.  Check spelling and capitalization.", null);
            int i;
            try {
    ...
    public static class FrontbaseExpression extends JDBCExpression {
        ...
        public String assembleSelectStatementWithAttributes(NSArray nsarray, boolean flag, EOQualifier eoqualifier, NSArray nsarray1, 
                                                            String clause, String columns, String table, String qualifier, String join, String order, String lock) {
        ...
        NSArray lobList() {
            return ((NSArray) (_lobList != null ? _lobList : NSArray.EmptyArray));
        }
        ...
        public String sqlStringForKeyValueQualifier(EOKeyValueQualifier eokeyvaluequalifier) {
            ...
            if (column == null)
                throw new IllegalStateException("sqlStringForKeyValueQualifier: attempt to generate SQL for " + 
                                                eokeyvaluequalifier.getClass().getName() + " " + 
                                                eokeyvaluequalifier + " failed because attribute identified by key '" + 
                                                attrubute + "' was not reachable from from entity '" + _entity.name() + "'");
            Object qualifier = eokeyvaluequalifier.value();
            if (qualifier instanceof EOQualifierVariable)
                throw new IllegalStateException("sqlStringForKeyValueQualifier: attempt to generate SQL for " + 
                                                eokeyvaluequalifier.getClass().getName() + " " + 
                                                eokeyvaluequalifier + " failed because the qualifier variable '$" + 
                                                ((EOQualifierVariable) qualifier).key() + "' is unbound.");
            ...
            boolean flag = nsselector.equals(EOQualifier.QualifierOperatorLike) || 
                           nsselector.equals(EOQualifier.QualifierOperatorCaseInsensitiveLike) || 
        ...
        public String formatValueForAttribute(Object obj, EOAttribute eoattribute) {
            if (obj != null && obj != NSKeyValueCoding.NullValue) {
                if (eoattribute.valueFactoryMethod() != null && 
                    eoattribute.valueFactoryMethod().implementedByObject(obj) && 
                    eoattribute.adaptorValueConversionMethod().implementedByObject(obj)) {
