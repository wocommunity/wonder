...
public class ERXSQLHelper {
	...
	public String createIndexSQLForEntities(NSArray entities, NSArray externalTypesToIgnore) {
		...
		for (Enumeration entitiesEnum = entities.objectEnumerator(); entitiesEnum.hasMoreElements();) {
			...
			for (Enumeration<String> keys = d.allKeys().objectEnumerator(); keys.hasMoreElements();) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public NSArray attributesToFetchForEntity(EOFetchSpecification fetchSpec, EOEntity entity) {
		NSArray attributes;
		if (!fetchSpec.fetchesRawRows()) {
			attributes = entity.attributesToFetch();
		}
		else {
			NSMutableArray rawRowAttributes = new NSMutableArray();
			for (Enumeration keyPathsEnum = fetchSpec.rawRowKeyPaths().objectEnumerator(); keyPathsEnum.hasMoreElements();) {
				String rawRowKeyPath = (String)keyPathsEnum.nextElement();
	 ...
	 * @return the EOSQLExpression which the EOFetchSpecification would use
	 */
	public EOSQLExpression sqlExpressionForFetchSpecification(EOEditingContext ec, EOFetchSpecification spec, long start, long end, NSArray attributes) {
	 ...
	 *            the sql expression to add a "group by" clause to
	 */
	public void addGroupByClauseToExpression(NSArray attributes, EOSQLExpression expression) {
		StringBuffer groupByBuffer = new StringBuffer();
		for (Enumeration attributesEnum = attributes.objectEnumerator(); attributesEnum.hasMoreElements();) {
	...
	protected ColumnIndex[] columnIndexesFromColumnNames(String... columnNames) {
		NSMutableArray columnIndexes = new NSMutableArray();
		for (String columnName : columnNames) {
			columnIndexes.addObject(new ColumnIndex(columnName));
		}
		return (ColumnIndex[])columnIndexes.objects();
	}
		...
		try {
			...
			try {
				...
				if (typeInfo != null) {
					...
					// Awesome.
					for (Enumeration keysEnum = typeInfo.allKeys().objectEnumerator(); keysEnum.hasMoreElements();) {
						String possibleTypeInfoStringRep = (String)keysEnum.nextElement();
					...
					if (typeDescription != null) {
						defaultJDBCTypes = (NSArray) typeDescription.objectForKey("defaultJDBCType");
	...
	public static class FrontBaseSQLHelper extends ERXSQLHelper {
		...
		public String sqlForFullTextQuery(ERXFullTextQualifier qualifier, EOSQLExpression expression) {
			...
			NSArray terms = qualifier.terms();
			for (Enumeration termsEnum = terms.objectEnumerator(); termsEnum.hasMoreElements();) {
		...
		public void restoreConnectionSettingsAfterSchemaChange(EOEditingContext ec, EOModel model) {
			...
			NSArray urlComponents = NSArray.componentsSeparatedByString(url, "/");
			for (Enumeration urlComponentsEnum = urlComponents.objectEnumerator(); urlComponentsEnum.hasMoreElements();) {
				String urlComponent = (String)urlComponentsEnum.nextElement();
