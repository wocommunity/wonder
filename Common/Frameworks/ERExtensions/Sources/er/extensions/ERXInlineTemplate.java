 ...
 * <br />
 * The content which would usually go into the ".html" file within a
 * WOComponent's bundle, is specified using the "html" binding, the ".wod" part
 * is pecified by the "wod" binding. <br />
 * <br />
 * When using {@link WOOgnl} with "ognl.helperFunctions = true" and
 * "ognl.inlineBindings = true", you can leave out the WOD part. <br />
 * <br />
 * When keys are accessed, the component first determines the first element of
 * the path (e.g. key "foo" for path "foo.bar") and looks, if there is a binding
 * with that key. <br />
 * If there is such a binding, the value is retrieved and the rest of the
 * keyPath applied to it (valueForBinding("foo").valueForKeyPath("bar")). <br />
 * If no such binding is available, the value is stored in a
 * NSMutableDictionary. The remaining keyPath is applied as above. <br />
 * <br />
 * When an error occurs, an error message is displayed. The message can be
 * altered using the "errorTemplate" binding. <br />
 * <br />
 * Optionally, a "cacheKey" (String) can be specified, under which the parsed
 * WOElement will be cached. To allow updating, a "cacheVersion" (Object) is
 * available. When the version changes, the value is recalculated.
 * 
 ...
 * @binding cacheVersion Hint to determine if the cached object is up-to-date
 *          (optional)
 * @errorTemplate Template to use for displaying error messages. Uses
 *                {@link ERXSimpleTemplateParser} for display. Method name and
 *                HTML-escaped message are provided by the "method" and
 *                "message" keys. (optional)
 * 
 * @author th
 * 
 */
public class ERXInlineTemplate extends ERXNonSynchronizingComponent {
	...
	try {
		...
		if (keyPathComponents.count() > 0) {
		    ...
		    return NSKeyValueCodingAdditions.Utility.valueForKeyPath(o, remainingKeyPath);
		}
		if (log.isDebugEnabled())
		...
		return dynamicBindings().valueForKeyPath(keyPath);
	    }
	} catch (Throwable t) {
	    // save throwable
	    _deferredError = new Error("takeValueForKeyPath", t);
	...
	try {
		...
		if (cacheEntry != null && (requestedVersion == null || requestedVersion.equals(cacheEntry.version()))) { // requestedVersion
		    ...
		    if (log.isDebugEnabled())
			log.debug("updating cache: " + cacheKey + " / " + (cacheEntry == null ? null : cacheEntry.version())
				+ " -> " + requestedVersion);
		    element = _template();
	...
	WOElement element = WOComponent.templateWithHTMLString((templateHtmlString == null) ? "" : templateHtmlString,
		(templateWodString == null ? "" : templateWodString), null);
	return element;
    }
	...
	public String formatWithTemplate(String template) {
	    return ERXSimpleTemplateParser.sharedInstance().parseTemplateWithObject(template,
		    ERXSimpleTemplateParser.DEFAULT_DELIMITER, this);
	}
    }
}