...
public class ERXQ {
	...
	public static NSArray filtered(NSArray array, EOQualifier qualifier) {
	 ...
	 */
	public static <T> T one(NSArray array, EOQualifier qualifier) {
		T object;
		if (array == null) {
			object = null;
		}
		else {
			NSArray objects = ERXQ.filtered(array, qualifier);
			int count = objects.count();
			if (count == 0) {
				object = null;
			}
			else if (count == 1) {
				object = (T)objects.lastObject();
			}
			else {
	 ...
	 * @throw IllegalStateException if more than one object matched
	 */
	public static <T> T first(NSArray array, EOQualifier qualifier) {
		T object;
		if (array == null) {
			object = null;
		}
			...
			if (count == 0) {
				...
				object = (T)objects.objectAtIndex(0);
			}
		}
		return object;
	}
	 ...
	 */
	public static <T> T requiredOne(NSArray array, EOQualifier qualifier) {
		T object = (T)ERXQ.one(array, qualifier);
		if (object == null) {
	 ...
	 */
	public static ERXOrQualifier containsAny(NSArray keys, String tokensWithWhitespace) {
		NSMutableArray qualifiers = new NSMutableArray();
		for (Enumeration keysEnum = keys.objectEnumerator(); keysEnum.hasMoreElements();) {
	...
	public static ERXOrQualifier containsAny(String key, String[] tokens) {
		...
		if (tokens.length == 0) {
			...
			NSMutableArray searchQualifiers = new NSMutableArray();
	 ...
	 * @return an ERXOrQualifier
	 */
	public static ERXOrQualifier containsAll(NSArray keys, String tokensWithWhitespace) {
		NSMutableArray qualifiers = new NSMutableArray();
		for (Enumeration keysEnum = keys.objectEnumerator(); keysEnum.hasMoreElements();) {
			String key = (String)keysEnum.nextElement();
	...
	public static ERXAndQualifier containsAllInAny(String[] keys, String[] tokens) {
		...
		if (tokens.length == 0) {
			...
			NSMutableArray searchQualifiers = new NSMutableArray();
			for (String token : tokens) {
				NSMutableArray tokenQualifiers = new NSMutableArray();
				for (String key : keys) {
