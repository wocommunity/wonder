...
public class ERXFetchResultCache {
	...
	private ERXExpiringCache<String, NSArray> cache = new ERXExpiringCache<String, NSArray>() {
		@Override
		protected synchronized void removeEntryForKey(Entry<NSArray> entry, String key) {
			for (Enumeration gidsEnum = entry.object().objectEnumerator(); gidsEnum.hasMoreElements();) {
		...
		@Override
		protected synchronized void setEntryForKey(Entry<NSArray> entry, String key) {
	 ...
	 * @param fs
	 */
	public NSArray objectsForFetchSpecification(EODatabaseContext dbc, EOEditingContext ec, EOFetchSpecification fs) {
		String identifier = ERXFetchSpecification.identifierForFetchSpec(fs);
		synchronized (cache) {
			currentDatabase = dbc.database();
			NSArray gids = cache.objectForKey(identifier);
			NSArray result = null;
	 ...
	 */
	protected NSArray excludedEntities() {
	...
	protected long cacheTime(NSArray eos, EOFetchSpecification fs) {
		...
		for (Enumeration eosEnum = eos.objectEnumerator(); eosEnum.hasMoreElements();) {
            Object object = eosEnum.nextElement();
