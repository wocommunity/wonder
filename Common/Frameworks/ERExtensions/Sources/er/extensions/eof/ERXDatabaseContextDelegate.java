...
public class ERXDatabaseContextDelegate {
     ...
     * note that when using {@link er.extensions.logging.ERXPatternLayout} one can set the option to
    ...
    private class ReentranceProtector {
    	...
    	private NSMutableArray _accessing = new NSMutableArray();
			...
			try {
				...
				if (source instanceof AutoBatchFaultingEnterpriseObject) {
					...
					if (_handler.batchSizeForRelationship(ec, relationship) > 0) {
						...
						for (Enumeration candidatesEnum = candidates.objectEnumerator(); candidatesEnum.hasMoreElements();) {
                            Object o = candidatesEnum.nextElement();
						...
						if (eos.count() > 1) {
							...
							int cnt = 0;
							for(Enumeration faultsEnum = faults.objectEnumerator(); faultsEnum.hasMoreElements();) {
                                Object fault = faultsEnum.nextElement();
								if(!EOFaultHandler.isFault(fault)) {
			...
			try {
				...
				if(sourceGID != null && key != null) {
					...
					if(_handler.batchSizeForRelationship(ec, relationship) > 0 && !relationship.isToMany()) {
						...
						for (Enumeration candidatesEnum = candidates.objectEnumerator(); candidatesEnum.hasMoreElements();) {
                            EOEnterpriseObject current = (EOEnterpriseObject)candidatesEnum.nextElement();
