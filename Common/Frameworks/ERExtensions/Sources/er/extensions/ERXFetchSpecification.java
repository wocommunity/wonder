...
public class ERXFetchSpecification<T extends EOEnterpriseObject> extends EOFetchSpecification {
	 ...
	 * @return
	 */
	public NSArray fetchObjects(EOEditingContext ec) {
	 ...
	 * @return
	 */
	public NSArray fetchRawRows(EOEditingContext ec) {
			...
			ERXQualifierTraversal traversal = new ERXQualifierTraversal() {
				...
				protected boolean traverseKeyValueQualifier(EOKeyValueQualifier q) {
					...
					if (value instanceof EOEnterpriseObject) {
						...
						String s = "";
						for (Enumeration arrayEnum = arr.objectEnumerator(); arrayEnum.hasMoreElements();) {
	...
	public static String identifierForFetchSpec(EOFetchSpecification fs) {
		...
		for (Enumeration orderingsEnum = fs.sortOrderings().objectEnumerator(); orderingsEnum.hasMoreElements();) {
			EOSortOrdering so = (EOSortOrdering) orderingsEnum.nextElement();
