 ...
 * Use subclasses of EOCustomObjectClazz as inner classes in your EO subclasses
 ...
 * implementation.
 */
public class EOCustomObjectClazz extends Object {
    /**
     * logging support
     */
    public static final ERXLogger log = ERXLogger.getERXLogger(EOCustomObjectClazz.class);
     ...
     */
    private static EOCustomObjectClazz classFromEntity(EOEntity entity) {
        EOCustomObjectClazz clazz = null;
        if(entity == null) {
            return new EOCustomObjectClazz();
        }
        try {
            String className = entity.className();
            if(className.equals("ERXGenericRecord"))
                clazz = new ERXCustomObject.ERXCustomObjectClazz();
            else
                clazz = (EOCustomObjectClazz)Class.forName(className + "$" + entity.name() + "Clazz").newInstance();
     ...
     * @return clazz object for the given entity
     */
    public static EOCustomObjectClazz clazzForEntityNamed(String entityName) {
        EOCustomObjectClazz clazz = (EOCustomObjectClazz)allClazzes.objectForKey(entityName);
        if(clazz == null) {
    ...
    public Number objectCountWithQualifier(EOEditingContext ec, EOQualifier qualifier) {
        ...
        EOAttribute attribute = EOCustomObjectClazz.objectCountAttribute();
        EOEntity entity = entity(ec);
