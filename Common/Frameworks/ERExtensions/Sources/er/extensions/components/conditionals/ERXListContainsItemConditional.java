...
public class ERXListContainsItemConditional extends ERXWOConditional {
	...
	@Override
	protected void pullAssociations(NSDictionary dict) {
		_list = (WOAssociation)dict.objectForKey("list");
		_item = (WOAssociation)dict.objectForKey("item");
	...
	protected boolean conditionInComponent(WOComponent component) {
		Object list = _list.valueInComponent(component);
		Object item = _item.valueInComponent(component);
		if (list instanceof List) {
			return list != null && ((List)list).contains(item); 
		} else if (list instanceof NSArray) { // for WO < 5.4
			return list != null && ((NSArray)list).containsObject(item);
		}
		return false;
	}
}
