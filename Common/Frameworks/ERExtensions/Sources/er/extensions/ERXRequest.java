...
public  class ERXRequest extends WORequest {
    ...
    protected static final NSArray HOST_ADDRESS_KEYS = new NSArray(new String[]{"pc-remote-addr", "remote_host", "remote_addr", "remote_user", "x-webobjects-remote-addr"});

    protected static final NSArray HOST_NAME_KEYS = new NSArray(new String[]{"x-forwarded-host", "Host", "x-webobjects-server-name", "server_name", "http_host"});
    ...
    protected NSArray fixAbbreviationArray(NSArray languages) {
        ...
        NSMutableArray languagePrefix = new NSMutableArray(languages.count());
        for (int languageNum = languages.count() - 1; languageNum >= 0; languageNum--) {
            String language = (String)languages.objectAtIndex(languageNum);
            int offset;
    ...
    public String remoteHostAddress() {
        ...
        for (Enumeration<String> keysEnum = HOST_ADDRESS_KEYS.objectEnumerator(); keysEnum.hasMoreElements();) {
    ...
    public String remoteHostName() {
    	for (Enumeration<String> keysEnum = HOST_NAME_KEYS.objectEnumerator(); keysEnum.hasMoreElements();) {
			String key = keysEnum.nextElement();
