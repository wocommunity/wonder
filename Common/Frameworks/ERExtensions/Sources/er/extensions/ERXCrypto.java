...
public class ERXCrypto {
	...
	private static NSMutableDictionary _crypters;

	private static synchronized NSMutableDictionary crypters() {
		if (_crypters == null) {
			...
			NSArray crypterAlgorithms = ERXProperties.componentsSeparatedByString("er.extensions.ERXCrypto.crypters", ",");
			if (crypterAlgorithms != null) {
				for (Enumeration algorithmsEnum = crypterAlgorithms.objectEnumerator(); algorithmsEnum.hasMoreElements();) {
					String crypterAlgorithm = (String)algorithmsEnum.nextElement();
	...
	public static void setCrypterForAlgorithm(ERXCrypterInterface crypter, String algorithm) {
		NSMutableDictionary crypters = ERXCrypto.crypters();
	...
	public static NSMutableDictionary decodedFormValuesDictionary(NSDictionary dict) {
		...
		for (Enumeration e = dict.allKeys().objectEnumerator(); e.hasMoreElements();) {
			...
			String value = ERXCrypto.defaultCrypter().encrypt((String)objects.lastObject()).trim();
