...
(function($) {
	...
	var defaults = {
		...
		termParam : 'term',
		loadingHtml : '<li><div class="ui-loader"><span class="ui-icon ui-icon-loading"></span></div></li>',
		interval : 0,
		builder: null,
		dataHandler : null,
		klass: null,
			...
			if (data) {
				...
				$.each(data, function(index, value) {
					...
					if ($.isPlainObject(value)) {
						...
						str.push('<li ' + vclass + '><a href="' + settings.link + encodeURIComponent(value.value) + '" data-transition="' + settings.transition + '" data-autocomplete=\'' + JSON.stringify(value).replace(/'/g, "&#39;") + '\'>' + settings.labelHTML(value.label) + '</a></li>');
					} else {
						str.push('<li ' + vclass + '><a href="' + settings.link + encodeURIComponent(value) + '" data-transition="' + settings.transition + '">' + settings.labelHTML(value) + '</a></li>');
					}
				});
			}
		}
