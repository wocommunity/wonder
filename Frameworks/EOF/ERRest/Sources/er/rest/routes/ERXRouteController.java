...
public class ERXRouteController extends WODirectAction {
	...
	public String routeStringForKey(String key) {
		return (String)_routeKeys.objectForKey(new ERXRoute.Key(key));
	}
	...
	public boolean containsRouteKey(String key) {
		return /*_routeKeys.containsKey(new ERXRoute.Key(key))*/_routeKeys.objectForKey(new ERXRoute.Key(key)) != null;
	}
	...
	protected void _takeRouteParametersFromRequest(WOActionResults results) {
		...
		for (Enumeration routeKeysEnum = _routeKeys.keyEnumerator(); routeKeysEnum.hasMoreElements();) {
			ERXRoute.Key key = (ERXRoute.Key)routeKeysEnum.nextElement();
	...
	public static void _disposeControllersForRequest(WORequest request) {
		...
		if (controllers != null) {
			...
			for (Enumeration controllerEnum = controllers.objectEnumerator(); controllerEnum.hasMoreElements(); ) {
				ERXRouteController controller = (ERXRouteController)controllerEnum.nextElement();
				controller.dispose();
			}
		}
	}
