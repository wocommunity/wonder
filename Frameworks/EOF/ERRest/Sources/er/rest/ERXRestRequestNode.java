...
public class ERXRestRequestNode implements NSKeyValueCoding, NSKeyValueCodingAdditions {
	...
	protected Object toJavaCollection(ERXRestFormat.Delegate delegate, Map<Object, ERXRestRequestNode> conversionMap, Map<Object, Object> associatedObjects) {
		...
		if (result == null) {
			...
			if (isArray()) {
				...
				for (Enumeration childEnum = _children.objectEnumerator(); childEnum.hasMoreElements(); ) {
	...
	protected Object toNSCollection(ERXRestFormat.Delegate delegate, NSMutableDictionary/*<Object, Object>*/ associatedObjects) {
		...
		if (result == null) {
			...
			if (isArray()) {
				...
				for (Enumeration childEnum = _children.objectEnumerator(); childEnum.hasMoreElements(); ) {
					ERXRestRequestNode child = (ERXRestRequestNode)childEnum.nextElement();
					array./*add*/addObject(child.toNSCollection(delegate, associatedObjects));
				}
				result = array;
			}
			else if (isNull()) {
			...
			if (_associatedObject != null) {
				/*associatedObjects.put(_associatedObject, result);*/
				associatedObjects.setObjectForKey(result, _associatedObject);
			}
		}
		return result;
	}
	...
	public ERXRestRequestNode removeChildNamed(String name) {
		...
		if (node != null) {
			_children./*remove*/removeObject(node);
		}
		return node;
	}
	...
	protected void guessNull() {
		setNull(_value == null && _children./*size*/count() == 0 && _id == null && !isArray() && _associatedObject == null);
	}
	...
	public Object objectWithFilter(String entityName, ERXKeyFilter keyFilter, IERXRestDelegate delegate) {
		...
		if (isArray()) {
			...
			for (Enumeration childEnum = children().objectEnumerator(); childEnum.hasMoreElements(); ) {
				ERXRestRequestNode childNode = (ERXRestRequestNode)childEnum.nextElement();
				Object child = childNode.objectWithFilter(entityName, ERXKeyFilter.filterWithAllRecursive(), delegate);
				if (child != null) {
			...
			obj = delegate.objectOfEntityWithID(classDescription, id());
			if (keyFilter != null) {
	...
	protected void _addAttributesAndRelationshipsForObjectOfEntity(Object obj, EOClassDescription classDescription, ERXKeyFilter keyFilter, IERXRestDelegate delegate, Set<Object> visitedObjects) {
		...
		for (Enumeration attributeNameEnum = classDescription.attributeKeys().objectEnumerator(); attributeNameEnum.hasMoreElements(); ) {
		...
		for (Enumeration relationshipNameEnum = classDescription.toOneRelationshipKeys().objectEnumerator(); relationshipNameEnum.hasMoreElements(); ) {
			String relationshipName = (String)relationshipNameEnum.nextElement();
		...
		for (Enumeration relationshipNameEnum = classDescription.toManyRelationshipKeys().objectEnumerator(); relationshipNameEnum.hasMoreElements(); ) {
			String relationshipName = (String)relationshipNameEnum.nextElement();
			// if (relationship.isClassProperty()) {
			ERXKey<Object> key = new ERXKey<Object>(relationshipName);
	...
	protected void _fillInWithObjectAndFilter(Object obj, EOClassDescription classDescription, ERXKeyFilter keyFilter, IERXRestDelegate delegate, Set<Object> visitedObjects) {
		...
		/* 5.2 Blerg */
			...
			for (Enumeration childObjEnum = ((NSArray)obj).objectEnumerator(); childObjEnum.hasMoreElements(); ) {
				Object childObj = childObjEnum.nextElement();
				ERXRestRequestNode childNode = new ERXRestRequestNode(null, false);
				childNode._fillInWithObjectAndFilter(childObj, classDescription, keyFilter, delegate, visitedObjects);
				addChild(childNode);
			}
	...
	public void updateObjectWithFilter(Object obj, ERXKeyFilter keyFilter, IERXRestDelegate delegate) {
		...
		for (Enumeration childEnum = _children.objectEnumerator(); childEnum.hasMoreElements(); ) {
			...
			if (isClassProperty(classDescription, keyName)) {
				...
				if ((List.class.isAssignableFrom(valueType) || NSArray.class.isAssignableFrom(valueType) /* 5.2 Blerg */) && keyFilter.matches(key, ERXKey.Type.ToManyRelationship)) {
					...
					for (Enumeration toManyNodeEnum = childNode.children().objectEnumerator(); toManyNodeEnum.hasMoreElements(); ) {
						ERXRestRequestNode toManyNode = (ERXRestRequestNode)toManyNodeEnum.nextElement();
	 ...
	 */
	public static ERXRestRequestNode requestNodeWithObjectAndFilter(EOClassDescription classDescription, /*List<?>*/NSArray objects, ERXKeyFilter keyFilter, IERXRestDelegate delegate) {
		String entityName = classDescription.entityName();
		ERXRestRequestNode requestNode = new ERXRestRequestNode(entityName, true);
	...
	public static ERXRestRequestNode requestNodeWithObjectAndFilter(Object obj, ERXKeyFilter keyFilter, IERXRestDelegate delegate) {
		...
		if (ERXRestUtils.isPrimitive(obj)) {
			...
			if (!(obj instanceof List || obj instanceof NSArray /* 5.2 Blerg */)) {
