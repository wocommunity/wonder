...
public class ERXGianduiaRestWriter implements IERXRestWriter {
	...
	protected void appendObjectToResponse(Object object, Map<Object, ERXRestRequestNode> conversionMap, IERXRestResponse response) {
		...
		/* 5.2 Blerg */
			...
			appendNSDictionaryToResponse((NSDictionary) object, conversionMap, response);
		}
		...
		/* 5.2 Blerg */
			...
			appendNSArrayToResponse((NSArray) object, conversionMap, response);
		}
		else {
	...
	protected void appendNSArrayToResponse(NSArray list, Map<Object, ERXRestRequestNode> conversionMap, IERXRestResponse response) {
		boolean persistentStoreFormat = false;
		Object id = null;
		if (_persistentStoreFormat) {
			ERXRestRequestNode requestNode = conversionMap.get(list);
		...
		response.appendContentString("[");
		Enumeration listIter = list.objectEnumerator();
		while (listIter.hasMoreElements()) {
			Object obj = listIter.nextElement();
			appendObjectToResponse(obj, conversionMap, response);
			if (listIter.hasMoreElements()) {
				response.appendContentString(", ");
			}
		}
		response.appendContentString("]");
	...
	/* 5.2 Blerg */
	protected void appendNSDictionaryToResponse(NSDictionary map, Map<Object, ERXRestRequestNode> conversionMap, IERXRestResponse response) {
		NSDictionary cleanedMap = map;
		boolean persistentStoreFormat = false;
		if (_persistentStoreFormat) {
			NSMutableDictionary mutableCleanedMap = map.mutableClone();
			String type = (String) mutableCleanedMap.removeObjectForKey(ERXRestFormatDelegate.TYPE_KEY);
			Object id = mutableCleanedMap.removeObjectForKey(ERXRestFormatDelegate.ID_KEY);
			persistentStoreFormat = (type != null && id != null);
			if (persistentStoreFormat) {
				response.appendContentString("this.objectWithURIRepresentation(\"x-coredata://" + _persistentStoreName + "/" + type + "/p" + id + "\",");
			}
			cleanedMap = mutableCleanedMap;
		}
		response.appendContentString("{");
		for (Enumeration keyEnum = cleanedMap.keyEnumerator(); keyEnum.hasMoreElements(); ) {
			Object key = keyEnum.nextElement();
			response.appendContentString(String.valueOf(key));
			response.appendContentString(":");
			appendObjectToResponse(cleanedMap.objectForKey(key), conversionMap, response);
