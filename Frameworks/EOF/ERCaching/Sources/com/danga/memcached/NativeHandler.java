...
public class NativeHandler {
    ...
    // logger
    private Logger log = Logger.getLogger(NativeHandler.class.getName());

    protected static NativeHandler handler = new NativeHandler();

    public static Object decode(byte[] b, int flag) throws Exception {
        return handler.doDecode(b, flag);
    }
    ...
    public static int getMarkerFlag(Object value) {
        return handler.doGetMarkerFlag(value);
    }

    public static boolean isHandled(Object value) {
        return handler.doIsHandled(value);
    }
     ...
     * @return true/false
     */
    protected boolean doIsHandled(Object value) {
        ...
        return (value instanceof Byte || value instanceof Boolean || value instanceof Integer || value instanceof Long || value instanceof Character || value instanceof String
                || value instanceof StringBuffer || value instanceof Float || value instanceof Short || value instanceof Double || value instanceof Date
     ...
     * @return int marker
     */
    protected int doGetMarkerFlag(Object value) {
     ...
     *             If fail to encode.
     */
    protected byte[] doEncode(Object value) throws Exception {
    ...
    protected byte[] encode(Byte value) {
        byte[] b = new byte[1];
        b[0] = value.byteValue();
        return b;
    }

    protected byte[] encode(Boolean value) {
        byte[] b = new byte[1];
    ...
    protected byte[] encode(int value) {
        return getBytes(value);
    }

    protected byte[] encode(long value) throws Exception {
        return getBytes(value);
    }

    protected byte[] encode(Date value) {
        return getBytes(value.getTime());
    }

    protected byte[] encode(Character value) {
        return encode(value.charValue());
    }

    protected byte[] encode(String value) throws Exception {
        return value.getBytes("UTF-8");
    }

    protected byte[] encode(StringBuffer value) throws Exception {
        return encode(value.toString());
    }

    protected byte[] encode(float value) throws Exception {
        return encode((int) Float.floatToIntBits(value));
    }

    protected byte[] encode(Short value) throws Exception {
        return encode((int) value.shortValue());
    }

    protected byte[] encode(double value) throws Exception {
        return encode((long) Double.doubleToLongBits(value));
    }

    protected byte[] encode(StringBuilder value) throws Exception {
        return encode(value.toString());
    }
     ...
     * @throws Exception
     */
    protected Object doDecode(byte[] b, int flag) throws Exception {
    ...
    // decode methods
    protected Byte decodeByte(byte[] b) {
        return new Byte(b[0]);
    }

    protected Boolean decodeBoolean(byte[] b) {
        boolean value = b[0] == 1;
        return (value) ? Boolean.TRUE : Boolean.FALSE;
    }

    protected Integer decodeInteger(byte[] b) {
        return new Integer(toInt(b));
    }

    protected Long decodeLong(byte[] b) throws Exception {
    ...
    protected String decodeString(byte[] b) throws Exception {
        return new String(b, "UTF-8");
    }

    protected StringBuffer decodeStringBuffer(byte[] b) throws Exception {
        return new StringBuffer(decodeString(b));
    }

    protected Float decodeFloat(byte[] b) throws Exception {
        Integer l = decodeInteger(b);
        return new Float(Float.intBitsToFloat(l.intValue()));
    }

    protected Short decodeShort(byte[] b) throws Exception {
        return new Short((short) decodeInteger(b).intValue());
    }

    protected Double decodeDouble(byte[] b) throws Exception {
        Long l = decodeLong(b);
        return new Double(Double.longBitsToDouble(l.longValue()));
    }

    protected Date decodeDate(byte[] b) {
        return new Date(toLong(b));
    }

    protected StringBuilder decodeStringBuilder(byte[] b) throws Exception {
        return new StringBuilder(decodeString(b));
    }

    protected byte[] decodeByteArr(byte[] b) {
        return b;
    }
