 ...
 * to display block content. In fact, buttons nested inside anchors
 * no longer validates with HTML5. So, this element is a re-implementation
 * of the ERXLinkButton dynamic element that renders either an anchor,
 * or a button, but not both.
 * 
...
public class ERXLinkButton5 extends WOHTMLDynamicElement {
	...
	protected void _appendButtonAttributesToResponse(WOResponse response, WOContext context) {
		_appendTagAttributeAndValueToResponse(response, "type", "submit", false);
		if(isDisabledInContext(context)) {
			_appendDisabledAttributeToResponse(response, context);
		} else {
			_appendNameAttributeToResponse(response, context);
	...
	protected void _appendIEOpenTagToResponse(WOResponse response, WOContext context) {
		response.appendContentString("<!--[if lt IE 8]>");
	}
	...
	protected void _appendNotIEOpenTagToResponse(WOResponse response, WOContext context) {
		response.appendContentString("<!--[if gte IE 8]> <-->");
	}
