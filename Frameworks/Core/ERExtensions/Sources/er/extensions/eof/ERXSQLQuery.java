...
public class ERXSQLQuery {
     ...
     * </ul>
     * <p>
     * <strong>NOTE:</strong> On the 5.2 version, this method will not respect model-based type conversions when using
     * {@link ERXKeyValueBinding}. That feature is only available on 5.4.
     * </p>
     * 
     * @param ec
     *          The current editing context (used to obtain the correct OSC)
    ...
    public static NSArray rawRowsForQuery( EOEditingContext ec, String modelName, String query, ERXSQLBinding... bindings ) {
        ...
        try{
            ...
            NSMutableArray resultList = new NSMutableArray(100);
            
            try {
                statement = connection.prepareStatement(expression.statement());
                
                int index = 1;
                for (Enumeration bindingEnumerator = expression.bindVariableDictionaries().objectEnumerator(); bindingEnumerator.hasMoreElements(); ) {
                    NSDictionary binding = (NSDictionary) bindingEnumerator.nextElement();
                ...
                NSMutableArray columnNames = new NSMutableArray(numColumns);
