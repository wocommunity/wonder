...
/**
 * overrides OracleExpression in order to add TIMESTAMP values including
 * milliseconds. The normal EOF Oracle PlugIn does not add milliseconds to the
 * TIMESTAMP value
 * 
 * @author David Teran
 *
...
public class EROracleExpression extends OracleExpression {
	...
	public interface Delegate {
		 ...
		 */
		String constraintStatementForRelationship(EORelationship relationship, NSArray sourceColumns,
	 ...
	 */
	public static void setDelegate(final EROracleExpression.Delegate delegate) {
	...
	public EROracleExpression(final EOEntity eoentity) {
		super(eoentity);
	}

	@Override
	public void addCreateClauseForAttribute(final EOAttribute attribute) {
		final NSDictionary userInfo = attribute.userInfo();
		...
		String sql;
		final String allowsNullClauseForConstraint = allowsNullClauseForConstraint(shouldAllowNull(attribute));
		if (defaultValue == null) {
			sql = _NSStringUtilities.concat(attribute.columnName(), " ", columnTypeStringForAttribute(attribute), " ",
					allowsNullClauseForConstraint);
		} else {
			sql = _NSStringUtilities.concat(attribute.columnName(), " ", columnTypeStringForAttribute(attribute),
					" DEFAULT ", formatValueForAttribute(defaultValue, attribute), " ", allowsNullClauseForConstraint);
		}
	 ...
	 * 
	 * If you want to use either NullsFirst or NullsLast, you will need to create a
	 * new EOSortOrdering.ComparisonSupport class and set it to be used at
	 * application startup otherwise EOF will still go and resort using nulls as the
	 * smallest value.
	 * 
	 * @see com.webobjects.eoaccess.EOSQLExpression#addOrderByAttributeOrdering(com.webobjects.eocontrol.EOSortOrdering)
	 */
	@Override
	public void addOrderByAttributeOrdering(final EOSortOrdering sortOrdering) {
		super.addOrderByAttributeOrdering(sortOrdering);
		final String nullSortBehavior = NSProperties.getProperty("EROraclePlugin.nullSortBehavior");
		if (nullSortBehavior != null) {
			switch (nullSortBehavior) {
			case "EOFStyle":
	 ...
	 * DISTINCT fetches with compareCaseInsensitiveA(De)scending sort orders. Lifted
	 * directly from the PostgressExpression.java class.
	 * 
	 ...
	 */
	@Override
	public String assembleSelectStatementWithAttributes(final NSArray attributes, final boolean lock,
			final EOQualifier qualifier, final NSArray fetchOrder, final String selectString, final String columnList,
			final String tableList, final String whereClause, final String joinClause, final String orderByClause,
		...
		if (selectString.indexOf(" DISTINCT") != -1) {
			final String[] columns = orderByClause.split(",");
			for (final String column2 : columns) {
				String column = column2.replaceFirst("\\s+(ASC|DESC)\\s*", "");
	...
	/**
	 * Overridden in order to add milliseconds to the value. This applies only if
	 * obj is an instance of NSTimestamp and if valueType from the eoattribute is T
	 * 
	 * @param obj
	 * @param eoattribute
	 * 
	 ...
	 */
	@Override
	public NSMutableDictionary bindVariableDictionaryForAttribute(final EOAttribute eoattribute, final Object obj) {
		final NSMutableDictionary result = super.bindVariableDictionaryForAttribute(eoattribute, obj);
		if ((obj instanceof final NSTimestamp nstimestamp) && (isTimestampAttribute(eoattribute))) {
			final long millis = nstimestamp.getTime();
	...
	/**
	 * @return true to indicate that the Oracle jdbc driver should use bind
	 *         variables
	 */
	@Override
	public boolean mustUseBindVariableForAttribute(final EOAttribute attribute) {
		return true;
	}

	@Override
	public void prepareConstraintStatementForRelationship(final EORelationship relationship,
			final NSArray sourceColumns, final NSArray destinationColumns) {
		final EOEntity entity = relationship.entity();
		String tableName = entity.externalName();
		final int lastDot = tableName.lastIndexOf('.');
		if (lastDot >= 0) {
			tableName = tableName.substring(lastDot + 1);
		}
		String constraintName = null;
		if (EROracleExpression._delegate != null) {
			constraintName = EROracleExpression._delegate.constraintStatementForRelationship(relationship,
					sourceColumns, destinationColumns);
		}
		if (constraintName == null && entity != null) {
			constraintName = System.getProperty(
					"er.extensions.ERXModelGroup." + entity.name() + "." + relationship.name() + ".foreignKey");
		}
		if (constraintName == null) {
			constraintName = _NSStringUtilities.concat(tableName, "_", relationship.name(), "_FK");
		}
		final String sourceKeyList = sourceColumns.componentsJoinedByString(", ");
		final String destinationKeyList = destinationColumns.componentsJoinedByString(", ");

		final EOModel sourceModel = entity.model();
		final EOModel destModel = relationship.destinationEntity().model();
		if (sourceModel != destModel && !sourceModel.connectionDictionary().equals(destModel.connectionDictionary())) {
			throw new IllegalArgumentException(
					"prepareConstraintStatementForRelationship unable to create a constraint for " + relationship.name()
							+ " because the source and destination entities reside in different databases");
		}
		setStatement("ALTER TABLE " + entity.externalName() + " ADD CONSTRAINT " + constraintName + " FOREIGN KEY ("
				+ sourceKeyList + ") REFERENCES " + relationship.destinationEntity().externalName() + " ("
				+ destinationKeyList + ") DEFERRABLE INITIALLY DEFERRED");
	}

	protected boolean shouldAllowNull(final EOAttribute attribute) {
		boolean shouldAllowNull = attribute.allowsNull();
		// If you allow nulls, then there's never a problem ...
		if (!shouldAllowNull) {
			final EOEntity entity = attribute.entity();
			final EOEntity parentEntity = entity.parentEntity();
			final String externalName = entity.externalName();
			if (externalName != null) {
				// If you have a parent entity and that parent entity shares your table name,
				// then you're single table inheritance
				final boolean singleTableInheritance = (parentEntity != null
						&& externalName.equals(parentEntity.externalName()));
				if (singleTableInheritance) {
					final EOAttribute parentAttribute = parentEntity.attributeNamed(attribute.name());
	 ...
	 */
	@Override
	public boolean shouldUseBindVariableForAttribute(final EOAttribute attribute) {
		return true;
	}
