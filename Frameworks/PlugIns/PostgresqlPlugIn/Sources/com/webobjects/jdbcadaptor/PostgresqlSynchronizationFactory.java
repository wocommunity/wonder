 ...
 * @author giorgio_v
 */
public class PostgresqlSynchronizationFactory extends EOSchemaSynchronizationFactory implements EOSchemaGeneration, EOSchemaSynchronization {
	...
	protected String formatTableName(String name) {
		...
		// Needs to replicate functionality of
		// EOSQLExpression.sqlStringForSchemaObjectName(String name). For
		// example MySchema.MyTable needs to be quoted "MySchema"."MyTable"
	...
	protected boolean _shouldGenerateForeignKeyConstraints(EORelationship rel) {
		...
		return !rel.isFlattened() && (destinationEntity.externalName() != null) && (rel.entity()
		                                                                               .model() == destinationEntity.model());
	}
	 ...
	 * 
	 * @param entity
	 *            the entity to which <code>PostgresqlExpression</code> is to be
	 *            rooted
	 * @param statement
	 *            the SQL statement
	 * @return a <code>PostgresqlExpression</code> rooted to <code>entity</code>
	 */
	private static PostgresqlExpression createExpression(EOEntity entity,
	                                                     String statement) {
	...
	public NSArray<EOSQLExpression> dropPrimaryKeySupportStatementsForEntityGroup(NSArray<EOEntity> entityGroup) {
		...
		for (EOEntity entity : entityGroup) {
			...
			if (!sequenceNames.containsObject(sequenceName)) {
				...
				results.addObject(PostgresqlSynchronizationFactory.createExpression(entity,
				                                                                    sql));
			}
		}
		return results;
	...
	public NSArray<EOSQLExpression> dropTableStatementsForEntityGroup(NSArray<EOEntity> entityGroup) {
		...
		for (EOEntity entity : entityGroup) {
			...
			if (PostgresqlSynchronizationFactory.entityUsesSeparateTable(entity)) {
				PostgresqlExpression result = new PostgresqlExpression(entity);
	...
	public NSArray<EOSQLExpression> foreignKeyConstraintStatementsForRelationship(EORelationship relationship) {
		...
		for (EOSQLExpression expression : superResults) {
			...
			s = PostgresqlSynchronizationFactory.replaceStringByStringInString(") INITIALLY DEFERRED",
			...
			String tableName = expression.sqlStringForSchemaObjectName(expression.entity()
			                                                                     .externalName());
			s = PostgresqlSynchronizationFactory.replaceStringByStringInString("ALTER TABLE " + tableNameWithoutSchemaName,
			                                                                   "ALTER TABLE " + tableName,
			                                                                   s);
			expression.setStatement(s);
			NSArray<String> columnNames = ((NSArray<String>) relationship.sourceAttributes()
			...
			results.addObject(PostgresqlSynchronizationFactory.createExpression(expression.entity(),
			                                                                    "CREATE INDEX " + indexName + " ON " + tableName + "( " + sbColumnNames.toString() + " )"));
		}
		return results;
	}
		...
		for (EOEntity entity : entityGroup) {
			...
			if (priKeyAttributes.count() == 1) {
				...
				if (!sequenceNames.containsObject(sequenceName)) {
					...
					// In Entity Modeler, we want to skip over the drop
					// statement
			...
			for (EOEntity entity : entityGroup) {
				for (EOAttribute attribute : entity.attributes()) {
					...
					if (!attribute.isDerived() && !attribute.isFlattened() && (columnName != null) && (columnName.length() > 0) && !columnNames.contains(columnName)) {
						sqlExpr.appendItemToListString(_columnCreationClauseForAttribute(attribute),
						                               aStatement);
						columnNames.addObject(columnName);
					}
				}
			}
			return new NSArray<EOSQLExpression>(_expressionForString(new StringBuilder().append("CREATE TABLE ")
			                                                                            .append(formatTableName(entityGroup.objectAtIndex(0)
			                                                                                                               .externalName()))
			                                                                            .append(" (")
			                                                                            .append(aStatement.toString())
			                                                                            .append(')')
			                                                                            .toString()));
		}
		return NSArray.EmptyArray;
	...
	// I blame statementstToConvertColumnType for not taking a damn EOAttribute
	// for
	...
	public String columnTypeStringForAttribute(EOAttribute attribute) {
		if (attribute.precision() != 0) {
			...
			return _NSStringUtilities.concat(attribute.externalType(),
			                                 "(",
			                                 precision,
			                                 ",",
			                                 scale,
	...
	public NSArray<EOSQLExpression> statementsToConvertColumnType(String columnName,
		...
		// if (options != null) {
		// String usingExpression = (String)
		// options.objectForKey(PostgresqlSynchronizationFactory.USING_KEY);
	...
	public NSArray<EOSQLExpression> statementsToInsertColumnForAttribute(EOAttribute attribute,
		...
		return new NSArray<EOSQLExpression>(_expressionForString("alter table " + formatTableName(attribute.entity()
		                                                                                                   .externalName()) + " add " + clause));
	}
	 ...
	 * expression.addCreateClauseForAttribute(eoattribute); return new
	 * StringBuffer(expression.listString()); }
	 * 
	 * public String _columnCreationClauseForAttribute(EOAttribute attribute) {
	 * return addCreateClauseForAttribute(attribute).toString(); }
	 */
	...
	public String schemaCreationScriptForEntities(NSArray allEntities,
		...
		NSArray statements = schemaCreationStatementsForEntities(allEntities,
		                                                         options);
		int i = 0;
	...
	public NSArray schemaCreationStatementsForEntities(NSArray allEntities,
		...
		boolean createDatabase = options.createDatabase();
		boolean dropDatabase = options.dropDatabase();
		if (createDatabase || dropDatabase) {
			...
			if (dropDatabase) {
				dropDatabaseStatements = dropDatabaseStatementsForConnectionDictionary(connectionDictionary,
				                                                                       null);
				if (dropDatabaseStatements == null) {
					dropDatabaseStatements = new NSArray(_expressionForString("/* The 'Drop Database' option is unavailable. */"));
				} else {
					adminCommentsNeeded = true;
				}
			}
			if (createDatabase) {
				createDatabaseStatements = createDatabaseStatementsForConnectionDictionary(connectionDictionary,
				                                                                           null);
				if (createDatabaseStatements == null) {
					createDatabaseStatements = new NSArray(_expressionForString("/* The 'Create Database' option is unavailable. */"));
				} else {
					adminCommentsNeeded = true;
				}
			}
		...
		if (options.foreignKeyConstraints()) {
			...
			for (int iCount = entityGroups.count(); i < iCount; i++) {
				result.addObjectsFromArray(_foreignKeyConstraintStatementsForEntityGroup((NSArray) entityGroups.objectAtIndex(i)));
			}
