...
/**
 * ERS3AttachmentProcessor implements storing attachments in Amazon's S3 service. For more information about configuring
 * an ERS3AttachmentProcessor, see the top level documentation.
 * 
...
public class ERS3AttachmentProcessor extends ERAttachmentProcessor<ERS3Attachment> {
	...
	private final ERS3UploadQueue _queue;
	...
	@Override
	public ERS3Attachment _process(EOEditingContext editingContext, File uploadedFile, String recommendedFileName,
			String mimeType, String configurationName, String ownerID, boolean pendingDelete) {
		boolean proxy = true;
		...
		if (bucket == null) {
			throw new IllegalArgumentException("There is no 'er.attachment." + configurationName
		...
		ERS3Attachment attachment = ERS3Attachment.createERS3Attachment(editingContext, Boolean.FALSE,
				new NSTimestamp(), mimeType, recommendedFileName, proxy, Integer.valueOf((int) uploadedFile.length()),
				null);
		if (delegate() != null) {
	...
	public String attachmentUrl(ERS3Attachment attachment, WORequest request, WOContext context) {
		...
		if (attachment.proxied()) {
			...
			try {
				...
				attachmentUrl = context.urlWithRequestHandlerKey(ERAttachmentRequestHandler.REQUEST_HANDLER_KEY,
						attachmentUrl, null);
	...
	public void deleteAttachment(ERS3Attachment attachment) throws MalformedURLException, IOException {
		...
		if (failed(response)) {
			throw new IOException("Failed to delete '" + bucket + "/" + key + "' to S3: Error "
					+ response.connection.getResponseCode() + ": " + response.connection.getResponseMessage());
		}
	}
	...
	public void attachmentInserted(ERS3Attachment attachment) {
		...
		_queue.enqueue((EOGlobalID) ERXEOControlUtilities.convertEOtoGID(attachment), attachment._pendingUploadFile(),
				attachment._isPendingDelete());
	}


	public void performUpload(File uploadedFile, String originalFileName, String bucket, String key, String mimeType,
			ERS3Attachment attachment) throws MalformedURLException, IOException {
		try {
			AWSAuthConnection conn = attachment.awsConnection();
			...
			try {
				...
				if (originalFileName != null) {
					headers.put("Content-Disposition",
							Arrays.asList(new String[] { "attachment; filename=" + originalFileName }));
				}

				Response response = conn.putStream(bucket, key, attachmentStreamObject, headers);
				if (failed(response)) {
					throw new IOException("Failed to write '" + bucket + "/" + key + "' to S3: Error "
							+ response.connection.getResponseCode() + ": " + response.connection.getResponseMessage());
				}
			} finally {
	...
	public class ERS3QueueEntry {
		...
		private final File _uploadedFile;

		private final ERS3Attachment _attachment;
	...
	public class ERS3UploadQueue extends ERXAsyncQueue<ERS3QueueEntry> {
		...
		public void enqueue(EOGlobalID attachment, File pendingUploadFile, boolean pendingDelete) {
			_editingContext.lock();
			try {
				ERS3Attachment localAttachment = (ERS3Attachment) ERXEOControlUtilities.convertGIDtoEO(_editingContext,
						attachment);
				localAttachment._setPendingUploadFile(pendingUploadFile, pendingDelete);
				ERS3QueueEntry entry = new ERS3QueueEntry(pendingUploadFile, localAttachment);
				enqueue(entry);
			} finally {
		...
		public void process(ERS3QueueEntry object) {
			...
			if (uploadedFile != null && uploadedFile.exists()) {
				...
				try {
					ERS3AttachmentProcessor.this.performUpload(uploadedFile, originalFileName, bucket, key, mimeType,
							attachment);
					...
					if (ERS3AttachmentProcessor.this.delegate() != null) {
						ERS3AttachmentProcessor.this.delegate().attachmentAvailable(ERS3AttachmentProcessor.this,
								attachment);
					}
				} catch (Throwable t) {
					if (ERS3AttachmentProcessor.this.delegate() != null) {
						ERS3AttachmentProcessor.this.delegate().attachmentNotAvailable(ERS3AttachmentProcessor.this,
								attachment, ERXExceptionUtilities.toParagraph(t));
					}
				...
				if (ERS3AttachmentProcessor.this.delegate() != null) {
					ERS3AttachmentProcessor.this.delegate().attachmentNotAvailable(ERS3AttachmentProcessor.this,
							attachment, "Missing attachment file '" + uploadedFile + "'.");
				}
