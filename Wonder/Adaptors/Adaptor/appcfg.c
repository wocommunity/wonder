...
static unsigned int config_interval = DEFAULT_CONF_INTERVAL;
...
static char *_retrieveServerInfo(ConfigServer *server, net_fd s, int *len, char *content_type) {
   ...
   resp = resp_getResponseHeaders(s);
   if (resp)
      resp = resp_getResponseContent(resp, s);
   if (resp != NULL) {
