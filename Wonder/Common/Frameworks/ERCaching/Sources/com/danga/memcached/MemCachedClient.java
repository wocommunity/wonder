 ...
 * href="http:/www.danga.com/memcached/">http://www.danga.com/memcached/</a>.
 * <br/> Supports setting, adding, replacing, deleting compressed/uncompressed
 * and<br/> serialized (can be stored as string if object is native class)
 * objects to memcached.<br/> <br/> Now pulls SockIO objects from SockIOPool,
 * which is a connection pool. The server failover<br/> has also been moved
 * into the SockIOPool class.<br/> This pool needs to be initialized prior to
 * the client working. See javadocs from SockIOPool.<br/> <br/> Some examples
 * of use follow.<br/>
 ...
 * </pre>
 * 
 * The set method shown above will always set the object in the cache.<br/> The
 * add and replace methods do the same, but with a slight difference.<br/>
 * <ul>
...
public class MemCachedClient {
    ...
    private static final String ITEM = "ITEM"; // start of item line from
     ...
     * requirements.
     * 
     * If enabled, data will be stored in compressed form if it is<br/> longer
     * than the threshold length set with setCompressThreshold(int)<br/> <br/>
     * The default is that compression is enabled.<br/> <br/> Even if
     * compression is disabled, compressed data will be automatically<br/>
     * decompressed.
     * 
     * @param compressEnable
     *            <CODE>true</CODE> to enable compression, <CODE>false</CODE>
     ...
     * optional hashcode.
     * 
     * The item is immediately made non retrievable.<br/> Keep in mind
     * {@link #add(String, Object) add} and
     * {@link #replace(String, Object) replace}<br/> will fail when used with
     * the same key will fail, until the server reaches the<br/> specified
     * time. However, {@link #set(String, Object) set} will succeed,<br/> and
     * the new value will not be deleted.
     * 
     * @param key
     *            the key to be removed
     * @param hashCode
     ...
     * is being<br/> deleted, the specified value will not be stored.<br/> The
     * server will automatically delete the value when the expiration time has
     * been reached.<br/> <br/> If compression is enabled, and the data is
     * longer than the compression threshold<br/> the data will be stored in
     * compressed form.<br/> <br/> As of the current release, all objects
     * stored will use java serialization.
     * 
     * @param cmdname
    ...
    private boolean set(String cmdname, String key, Object value, long expiry, Integer hashCode, boolean asString) {
        ...
        if (compressEnable && val.length > compressThreshold) {
            ...
            try {
                if (log.isDebugEnabled())
                    log.debug("++++ trying to compress data, size prior to compression: " + val.length);
        ...
        try {
            String cmd = String.format("%s %s %d %d %d\r\n", cmdname, key, flags, (expiry / 1000), val.length);
     ...
     * The server will only decrement a value if it already exists;<br/> if a
     * value is not found, -1 will be returned.
     * 
     * @param cmdname
     *            increment/decrement
     * @param key
