	...
	public void _verifyNoChangesToReadonlyEntity(EODatabaseOperation dbOp) {
		...
		if(dbOp.databaseOperator() == 2 && ((Boolean) NSKeyValueCoding.Utility.valueForKey(entity, "_hasNonUpdateableAttributes")).booleanValue()) {
			...
			for(int i = keys.count() - 1; i >= 0; i--) {
				...
				// FIX: ak when you have single-table inheritance and in the child there are foreign keys that are not in the parent
				// THEN, if the entity _hasNonUpdateableAttributes (public PK or read only props) the DB op is checked 
				// against the attributes. BUT this dictionary has all entries, even from the child (most likely NULL values)
				// and the current implementation doesn't check against the case when the attribute isn't present in the first place.
