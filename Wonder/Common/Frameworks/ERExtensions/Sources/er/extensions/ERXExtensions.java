...
/* ERXExtensions.java created by patrice on Thu 09-Mar-2000 */
	...
	if (s!=null) {
	  ...
	  a.addObject(ec);
          if (cat.isDebugEnabled()) cat.debug("Added new ec to array for "+s.sessionID());
            ...
            if (cat.isDebugEnabled()) cat.debug("Editing Context created with null session.");
  ...
  public static void sessionDidTimeOut(String sessionID) {
    if (sessionID!=null) {
      if (cat.isDebugEnabled()) {
          ...
          if (cat.isDebugEnabled()) cat.debug("Session "+sessionID+" is timing out ");
          if (cat.isDebugEnabled()) cat.debug("Found "+ ((a == null) ? 0 : a.count()) + " editing context(s)");
      }
    ...
    */
    ...
    public static void forceGC(int maxLoop) {
        ...
        // 1 bridge crossing to wrap up everything
        WOApplication.application().isDebuggingEnabled();
    }
